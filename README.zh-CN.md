...
默认情况下，镜像内的 HermesDeckX 服务会连接容器内本地 Gateway：`127.0.0.1:8642`。如果你需要改为连接宿主机或外部 Gateway，请在 `docker-compose.yml` 中覆盖 `OHD_HERMES_AGENT_GATEWAY_HOST` 与 `OHD_HERMES_AGENT_GATEWAY_PORT`。
...
| `8642` | HermesAgent Gateway | 可选：映射后可从容器外调试 Gateway |

如需从容器外访问 Gateway，在 `docker-compose.yml` 的 `ports` 中添加 `- "8642:8642"`。

注意，仅映射 `8642` 并不一定能从宿主机访问 Gateway。默认生成的最小 HermesAgent 配置会将 Gateway 绑定到 `loopback`，因此如果需要外部访问，可能还需要同步调整 Gateway 的绑定地址。
...
| `OHD_HERMES_AGENT_GATEWAY_PORT` | `8642` | Gateway 端口 |
