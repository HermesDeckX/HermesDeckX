...
export type WindowID =
  | 'dashboard' | 'gateway' | 'sessions' | 'activity'
  | 'usage' | 'editor' | 'skills' | 'agents' | 'maintenance'
  | 'scheduler' | 'settings' | 'setup_wizard' | 'usage_wizard'
  | 'knowledge';
 ...
 * All `hermesdeck:open-window` CustomEvents use this shape.
 */
...
export function dispatchOpenWindow(detail: OpenWindowDetail): void {
  window.dispatchEvent(new CustomEvent('hermesdeck:open-window', { detail }));
}
