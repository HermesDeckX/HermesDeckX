...
const CommandPalette: React.FC<CommandPaletteProps> = ({ language, openWindow }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('hermesdeck:open-palette', handler);
    return () => window.removeEventListener('hermesdeck:open-palette', handler);
  }, [openPalette]);
  ...
  const staticCommands = useMemo<Command[]>(() => {
    ...
    const actions: { id: string; label: string; icon: string; keywords: string; exec: () => void }[] = [
      ...
      {
        id: 'act:open-usage',
        label: cp.openUsage || 'Open Usage Statistics',
        icon: 'analytics',
        keywords: 'usage stats cost token analytics billing',
        exec: () => { openWindow('usage'); },
      },
      {
        id: 'act:open-scheduler',
        label: cp.openScheduler || 'Open Scheduler',
        icon: 'event_repeat',
        keywords: 'scheduler cron jobs automation task',
        exec: () => { openWindow('scheduler'); },
      },
      {
        id: 'act:open-knowledge',
        label: cp.openKnowledge || 'Open Knowledge Center',
        icon: 'auto_awesome',
        keywords: 'knowledge tips docs prompt memory guide',
        exec: () => { openWindow('knowledge'); },
      },
      {
      ...
      {
        id: 'act:open-editor-channels',
        label: cp.configChannels || 'Configure Channels',
        icon: 'forum',
        keywords: 'channels telegram discord slack whatsapp signal api server config',
        exec: () => { dispatchOpenWindow({ id: 'editor', section: 'channels' }); },
      },
      {
        id: 'act:open-editor-gateway',
        label: cp.configGateway || 'Configure Gateway',
        icon: 'dns',
        keywords: 'gateway approvals streaming privacy session config',
        exec: () => { dispatchOpenWindow({ id: 'editor', section: 'gateway' }); },
      },
      {
        id: 'act:open-editor-secrets',
        label: cp.configSecrets || 'Configure Secrets',
        icon: 'key',
        keywords: 'secrets tokens password api key env auth config',
        exec: () => { dispatchOpenWindow({ id: 'editor', section: 'secrets' }); },
      },
      {
        id: 'act:open-editor-cron',
        label: cp.configCron || 'Configure Scheduler Jobs',
        icon: 'schedule',
        keywords: 'cron scheduler jobs task automation config',
        exec: () => { dispatchOpenWindow({ id: 'editor', section: 'cron' }); },
      },
      {
