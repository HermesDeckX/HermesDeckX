...
export const UsagePanel: React.FC<UsagePanelProps> = ({ sessionKey, gwReady, loadUsage, loadTimeseries, labels: a, session: s, securityInfo: sec, onModelChange, onNavigateAgent, loadModels }) => {
  ...
  // Limited to a single retry to prevent infinite loops when the backend
  // consistently returns null (e.g. hermes-agent doesn't support sessions.usage).
  const hadNullUsage = useRef(false);
  const nullRetried = useRef(false);
  useEffect(() => {
    if (!usage && !loading && !error && gwReady && sessionKey) hadNullUsage.current = true;
    if (hadNullUsage.current && s?.totalTokens && !usage && !nullRetried.current) {
      hadNullUsage.current = false;
      nullRetried.current = true;
      loadData(true);
    }
  }, [s?.totalTokens, usage, loading, error, gwReady, sessionKey, loadData]);
  // Reset retry guard on session change
  useEffect(() => { nullRetried.current = false; }, [sessionKey]);
