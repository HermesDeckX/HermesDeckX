...
const Sessions: React.FC<SessionsProps> = ({ language, pendingSessionKey, onSessionKeyConsumed }) => {
  ...
  const SLASH_COMMANDS = useMemo(() => [
    ...
    { cmd: '/retry', desc: c.cmdRetry || 'Retry the last message', icon: 'replay', cat: 'session' },
    { cmd: '/undo', desc: c.cmdUndo || 'Remove last exchange', icon: 'undo', cat: 'session' },
    { cmd: '/title', desc: c.cmdTitle || 'Set session title', icon: 'title', cat: 'session' },
    { cmd: '/branch', desc: c.cmdBranch || 'Branch current session', icon: 'fork_right', cat: 'session' },
    ...
    { cmd: '/resume', desc: c.cmdResume || 'Resume a named session', icon: 'play_arrow', cat: 'session' },
    { cmd: '/rollback', desc: c.cmdRollback || 'Restore filesystem checkpoint', icon: 'restore', cat: 'session' },
    { cmd: '/agents', desc: c.cmdAgents || 'Show active agents and tasks', icon: 'groups', cat: 'session' },
    { cmd: '/sethome', desc: c.cmdSetHome || 'Set this chat as the home channel', icon: 'home', cat: 'session' },
    { cmd: '/restart', desc: c.cmdRestart || 'Gracefully restart the gateway', icon: 'restart_alt', cat: 'session' },
    // — Configuration —
    { cmd: '/model', desc: c.quickModel || 'Switch model', icon: 'smart_toy', cat: 'options' },
    { cmd: '/provider', desc: c.cmdProvider || 'Show providers', icon: 'cloud', cat: 'options' },
    { cmd: '/personality', desc: c.cmdPersonality || 'Set personality', icon: 'face', cat: 'options' },
    { cmd: '/verbose', desc: c.catOptions || 'Cycle tool progress display', icon: 'visibility', cat: 'options' },
    { cmd: '/yolo', desc: c.cmdYolo || 'Toggle YOLO mode (skip approvals)', icon: 'bolt', cat: 'options' },
    ...
    { cmd: '/deny', desc: c.cmdDeny || 'Deny pending command', icon: 'block', cat: 'status' },
  ], [c]);
