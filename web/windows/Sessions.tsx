...
const Sessions: React.FC<SessionsProps> = ({ language, pendingSessionKey, onSessionKeyConsumed }) => {
  ...
  const [sessions, setSessions] = useState<GwSession[]>(() => {
    try {
      const cached = sessionStorage.getItem('hermesdeck-sessions-cache');
  ...
  const SLASH_COMMANDS = useMemo(() => [
    // — Session —
    { cmd: '/new', desc: c.quickReset || 'Start a new session', icon: 'add_circle', cat: 'session' },
    { cmd: '/status', desc: c.quickStatus || 'Show session info', icon: 'info', cat: 'session' },
    { cmd: '/retry', desc: 'Retry the last message', icon: 'replay', cat: 'session' },
    { cmd: '/undo', desc: 'Remove last exchange', icon: 'undo', cat: 'session' },
    { cmd: '/title', desc: 'Set session title', icon: 'title', cat: 'session' },
    { cmd: '/branch', desc: 'Branch current session', icon: 'fork_right', cat: 'session' },
    { cmd: '/compress', desc: c.quickCompact || 'Compress context', icon: 'compress', cat: 'session' },
    { cmd: '/snapshot', desc: 'Create or restore snapshots', icon: 'inventory_2', cat: 'session' },
    ...
    { cmd: '/background', desc: 'Run prompt in background', icon: 'schedule', cat: 'session' },
    { cmd: '/resume', desc: 'Resume a named session', icon: 'play_arrow', cat: 'session' },
    { cmd: '/rollback', desc: 'Restore filesystem checkpoint', icon: 'restore', cat: 'session' },
    { cmd: '/restart', desc: 'Gracefully restart the gateway', icon: 'restart_alt', cat: 'session' },
    // — Configuration —
    { cmd: '/model', desc: c.quickModel || 'Switch model', icon: 'smart_toy', cat: 'options' },
    { cmd: '/provider', desc: 'Show providers', icon: 'cloud', cat: 'options' },
    { cmd: '/personality', desc: 'Set personality', icon: 'face', cat: 'options' },
    { cmd: '/verbose', desc: c.catOptions || 'Cycle tool progress display', icon: 'visibility', cat: 'options' },
    { cmd: '/yolo', desc: 'Toggle YOLO mode (skip approvals)', icon: 'bolt', cat: 'options' },
    { cmd: '/reasoning', desc: c.catOptions || 'Manage reasoning effort', icon: 'neurology', cat: 'options' },
    ...
    // — Tools & Skills —
    { cmd: '/reload', desc: 'Reload .env variables', icon: 'refresh', cat: 'tools' },
    { cmd: '/reload-mcp', desc: 'Reload MCP servers', icon: 'sync', cat: 'tools' },
    // — Info —
    { cmd: '/help', desc: c.quickHelp || 'Show commands', icon: 'help', cat: 'status' },
    { cmd: '/commands', desc: 'Browse available commands', icon: 'menu_book', cat: 'status' },
    { cmd: '/usage', desc: c.tokens || 'Token usage and rate limits', icon: 'data_usage', cat: 'status' },
    { cmd: '/insights', desc: 'Usage analytics', icon: 'analytics', cat: 'status' },
    { cmd: '/profile', desc: 'Show active profile', icon: 'badge', cat: 'status' },
    { cmd: '/debug', desc: 'Share a debug report', icon: 'bug_report', cat: 'status' },
    { cmd: '/config', desc: c.catManagement || 'Show configuration', icon: 'settings', cat: 'status' },
    ...
    const unsubscribe = subscribeManagerWS((msg: any) => {
      try {
        ...
        if (msg.type === 'chat') {
          ...
          // Gateway event: context compaction has begun for this session (hermesagent >=2026.3.24)
          const d = msg.data;
  ...
  const handleChatEvent = useCallback((payload?: any) => {
    ...
    if (payload.state === 'delta') {
      ...
      // Reject stale deltas that arrive after a run was already finalized
      // (e.g. hermes-agent synchronous response already injected).
      if (!pendingRunRef.current && finalizedAtRef.current && Date.now() - finalizedAtRef.current < 5000) {
        return;
      }
      ...
      // Surface provider-specific rate limit details from gateway (hermesagent >=2026.3.28)
  ...
  const handleAgentEvent = useCallback((payload?: any) => {
    ...
    if (payload.stream === 'tool') {
      ...
      const eventRunId = payload.runId as string | undefined;
      if (phase === 'start') {
        // Reject late lifecycle events for already-finalized runs
        if (eventRunId && finalizedRunsRef.current.has(eventRunId)) return;
        if (!pendingRunRef.current && finalizedAtRef.current && Date.now() - finalizedAtRef.current < 5000) return;
        setRunPhase('running');
  ...
  const loadSessions = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      setSessions(prev => {
        ...
        try { sessionStorage.setItem('hermesdeck-sessions-cache', JSON.stringify(merged)); } catch { /* ignore */ }
        return merged;
      });
    } catch { /* ignore */ }
    finally {
  ...
  // Helper: normalize timestamp — hermes-agent state.db stores Python time.time()
  // as seconds-level float (e.g. 1744522962.123), but JS Date expects milliseconds.
  const normalizeTs = (ts: number | undefined): number | undefined => {
    if (!ts) return ts;
    // Timestamps below 1e12 are in seconds (valid until year 33658 in ms)
  ...
  const mapMessages = useCallback((msgs: any[]): ChatMsg[] => msgs.map((m: any) => ({
    ...
    content: m.content,
    timestamp: normalizeTs(m.timestamp || m.ts),
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('hermesdeck-chat-drafts');
  ...
  const saveDraft = useCallback((key: string, text: string) => {
    ...
    try { localStorage.setItem('hermesdeck-chat-drafts', JSON.stringify(draftsRef.current)); } catch { /* ignore */ }
  }, []);
  ...
  const sendMessage = useCallback(async () => {
    ...
    // Build attachments for API — strip data URL prefix to send raw base64 (matches HermesAgent webchat protocol)
    ...
    try {
      ...
      const nextSessionKey = typeof res?.sessionKey === 'string' && res.sessionKey.trim()
        ? res.sessionKey.trim()
        : sessionKey;
      ...
      // hermes-agent returns the full response synchronously in res.response.
      // Inject the assistant message directly instead of waiting for event
      // streams or history polling (which hermes-agent doesn't support).
      const directResponse = typeof res?.response === 'string' ? res.response.trim() : '';
      if (directResponse) {
        ...
        // Mark run as finalized so late-arriving WS events (delta/lifecycle)
        // don't re-activate the streaming indicator after we've already got the response.
        if (nextRunId) {
          finalizedRunsRef.current.add(nextRunId);
          if (finalizedRunsRef.current.size > 100) {
            const first = finalizedRunsRef.current.values().next().value;
            if (first) finalizedRunsRef.current.delete(first);
          }
        ...
        pendingRunRef.current = null;
        void loadSessions();
        // Async: also reload history to pick up tool call details from state.db
        setTimeout(() => loadHistoryRef.current?.({ silent: true }), 3000);
      } else {
        // No direct response — fall back to waiting for events/history polling
        setRunId(nextRunId);
        setRunPhase('waiting');
        pendingRunRef.current = {
          runId: nextRunId,
      ...
      if (typeof errMsg === 'string' && /skill|tool|binary|not found/i.test(errMsg)) {
        gwApi.skillsBins().then(bins => {
          if (Array.isArray(bins) && bins.length === 0) {
            setError(prev => prev ? `${prev}  (no skill binaries installed — run "hermes skills install")` : prev);
          }
        }).catch(() => {});
      }
    } finally {
    ...
    try {
      ...
      if (peer && (peer.peerKind === 'group' || peer.peerKind === 'channel') && bindAgentId !== bindAgentOriginal) {
        try {
          ...
          // because HermesAgent's route matching is case-sensitive.
          if (bindAgentId) {
    ...
    <div className="flex-1 flex overflow-hidden bg-white dark:bg-[#0d1117] relative">
      ...
      <UsagePanel
        ...
        securityInfo={(() => {
          ...
          const enabledToolsets: string[] = Array.isArray(parsed?.enabled_toolsets) ? parsed.enabled_toolsets : [];
          const disabledToolsets: string[] = Array.isArray(parsed?.disabled_toolsets) ? parsed.disabled_toolsets : [];
          const maxIter = parsed?.max_iterations ?? 90;
          return {
            toolProfile: enabledToolsets.length > 0 ? enabledToolsets.join(', ') : 'all',
            sandboxMode: disabledToolsets.length > 0 ? disabledToolsets.join(', ') : '—',
            execSecurity: `max ${maxIter} iterations`,
            execHost: parsed?.platform || 'cli',
            execAsk: parsed?.quiet_mode ? 'quiet' : 'interactive',
          };
        })()}
        session={(() => {
