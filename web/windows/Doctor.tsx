...
import { Language, WindowID, dispatchOpenWindow } from '../types';
...
const Doctor: React.FC<DoctorProps> = ({ language }) => {
  ...
  const handleCliFix = useCallback(async () => {
    const shouldProceed = await confirm({
      ...
      message: text.cliFixConfirmMessage || 'Will run "hermes doctor --fix" locally. Continue?',
    ...
    try {
      ...
      if (code === 'CLI_NOT_INSTALLED') {
        toast('error', text.cliNotInstalled || 'hermes CLI is not installed');
      } else {
  ...
  const jumpToWindow = useCallback((id: string, opts?: { section?: string }) => {
    dispatchOpenWindow({ id: id as WindowID, ...opts });
  }, []);
  ...
  const summaryIssueAction = useCallback((source?: string, category?: string) => {
    ...
    if (key === 'alert') return { target: 'gateway', tab: 'events', label: text.actionReviewAlerts || 'Open Alerts' };
  ...
  const fallbackCheckCatalog = useMemo<Record<string, CheckLocaleEntry>>(() => {
    return {
      hermesagent_install: { name: 'Hermes Agent Install' }, config_file: { name: 'Config File' }, gateway_status: { name: 'Gateway Status' },
      pid_lock: { name: 'PID Lock' }, port_default: { name: 'Port Check' }, disk_space: { name: 'Disk Space' },
      gateway_diag_hermesagent_installed: { name: 'Hermes Agent Installed' }, gateway_diag_config_exists: { name: 'Config File Exists' },
  ...
  const checkDetailTpl = useMemo(() => {
    return {
      hermesagentInstalled: 'installed: {path}', hermesagentNotFound: 'hermesagent command not found',
  ...
  const localizeCheckItem = useCallback((item: CheckItem): LocalizedCheckItem => {
    ...
    if (code === 'hermesagent_install') {
      if (/^installed:\s*/i.test(detail)) detail = formatTpl(checkDetailTpl.hermesagentInstalled, { path: detail.replace(/^installed:\s*/i, '').trim() });
      else if (/hermesagent command not found/i.test(detail)) detail = checkDetailTpl.hermesagentNotFound;
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {activeTab === 'diagnose' && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleFix} disabled={fixing} className="h-8 px-3 rounded-lg text-[11px] font-bold bg-primary text-white disabled:opacity-40 flex items-center gap-1.5" title={text.cliFixTip || 'Run hermes doctor --fix locally'}>
                ...
                <button type="button" onClick={() => setSecurityAuditCollapsed(v => !v)}
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-start">
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-white/35">{text.securityAuditSubtitle || 'Hermes Agent security audit engine'}</p>
                    </div>
                  </div>
                          ...
                          <div className="mt-1.5 pt-1.5 border-t border-slate-100 dark:border-white/5">
                            <div className="flex items-center gap-1 flex-wrap">
                              ...
                              {relatedFaqs.map(faq => (
                                <button
                                  key={faq.id}
                                  onClick={() => dispatchOpenWindow({ id: 'knowledge', expandItem: faq.id })}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-[90%] max-w-2xl max-h-[70vh] flex flex-col border border-slate-200 dark:border-white/10" onClick={e => e.stopPropagation()}>
            <div className="px-4 py-3 border-b border-slate-200 dark:border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-[13px] font-bold text-slate-800 dark:text-white">hermes doctor --fix</h3>
              </div>
