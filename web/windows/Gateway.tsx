...
const Gateway: React.FC<GatewayProps> = ({ language }) => {
  ...
  const [formData, setFormData] = useState({ name: '', host: '127.0.0.1', port: 8642, token: '' });
  ...
  useEffect(() => {
    ...
    window.addEventListener('hermesdeck:open-window', handler as EventListener);
    return () => window.removeEventListener('hermesdeck:open-window', handler as EventListener);
  }, []);
  ...
  const fetchLogs = useCallback((force = false) => {
    ...
    gatewayApi.logTail(params).then((res) => {
      ...
      if (deepFilter || isInitial || res.reset) {
        ...
        setLogs(prev => {
          const uniqueNewLines = newLines.filter((line, index) => newLines.indexOf(line) === index && !prev.includes(line));
          const merged = [...prev, ...uniqueNewLines];
      ...
      if (isInitial) {
        gatewayApi.logCached(logLimit, 5000, force, logFile || undefined).then((res: any) => {
          let lines: string[] = [];
  ...
  useEffect(() => {
    ...
    if (prevRunning === true && !running && activeTab === 'logs') {
      fetchLogs(true);
    }
    prevRunningRef.current = running;
  }, [status?.running, activeTab, fetchLogs]);

  const fetchLogFiles = useCallback(() => {
    gatewayApi.logFiles().then((res) => {
      if (res?.files) setAvailableLogFiles(res.files);
    }).catch(() => {});
  }, []);
  ...
  // NOTE: fetchLogs is intentionally excluded — log polling is handled by useVisibilityPolling below.
  // Including it here caused the entire init effect to re-fire whenever logLimit/logFile changed,
  // producing duplicate /api/v1/gateway/log requests with different limit values.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetchProfiles, fetchStatus, fetchHealthCheck, fetchLogFiles, fetchEvents, fetchChannels, activeTab]);
  ...
  useVisibilityPolling(fetchLogs, 5000, activeTab === 'logs' && !!status?.running);
  ...
  const refreshAll = useCallback((force = false) => {
    ...
    fetchChannels(force);
    if (activeTab === 'logs') { logCursorRef.current = undefined; logInitializedRef.current = false; fetchLogs(force); fetchLogFiles(); }
    if (activeTab === 'events') fetchEvents();
  }, [activeTab, fetchProfiles, fetchStatus, fetchLogs, fetchLogFiles, fetchHealthCheck, fetchEvents, fetchChannels]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      if (editingProfile) {
        ...
        await gatewayProfileApi.create({ ...formData, port: formData.port || 8642 });
      }
      fetchProfiles(true);
      setEditingProfile(null);
      setFormData({ name: '', host: '127.0.0.1', port: 8642, token: '' });
      setFormErrors({});
                ...
                <div>
                  ...
                  <NumberStepper
                    ...
                    onChange={v => setFormData(f => {
                      const n = Number(v);
                      if (Number.isNaN(n)) return { ...f, port: 8642 };
              ...
              {['trace', 'debug', 'info', 'warn', 'error', 'fatal'].map(lvl => {
                ...
                return (
                  <button key={lvl} onClick={(e) => setLevelFilters(prev => {
                    ...
                    className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase transition-all ${!hasFilter ? `${colors[lvl]}/20 theme-text-secondary` : isActive ? `${colors[lvl]}/20 theme-text-secondary ring-1 ring-current` : 'theme-field theme-text-muted opacity-40'}`}>
                    {lvl.slice(0, 3)}
                  </button>
                );
              })}
