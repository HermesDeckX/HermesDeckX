...
const SnapshotTab: React.FC<SnapshotTabProps> = ({ s, inputCls, labelCls, rowCls }) => {
  ...
  const [backupMethod, setBackupMethod] = useState<'hermesdeckx' | 'hermesagent'>('hermesdeckx');
  const [manualSnapshotScope, setManualSnapshotScope] = useState<'both' | 'hermesagent' | 'hermesdeckx'>('both');
  ...
  const handleCreateSnapshot = async () => {
    ...
    if (backupMethod === 'hermesdeckx' && resourceIds.length === 0) { toast('error', s.snapshotScanSelectAtLeastOne || 'Select at least one file to back up'); return; }
    setSnapshotLoading(true);
    try { await snapshotApi.create({ password: snapshotPassword, trigger: 'manual', note: snapshotNote.trim() || undefined, scope: manualSnapshotScope, resourceIds: backupMethod === 'hermesdeckx' ? resourceIds : undefined }); toast('success', s.snapshotCreated || s.backupCreated); setSnapshotPassword(''); setSnapshotNote(''); refreshAll(); fetchSnapshotScan(manualSnapshotScope); }
  ...
  return (
    <>
      <div className="space-y-5">
        ...
        {snapshotModeTab === 'manual' && (<div className={rowCls}><div className="px-4 py-3 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 p-0.5 rounded-lg bg-slate-100 dark:bg-white/[0.05]">
              <button type="button" onClick={() => setBackupMethod('hermesdeckx')} className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${backupMethod === 'hermesdeckx' ? 'bg-white dark:bg-white/10 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-white/60 hover:text-slate-700 dark:hover:text-white/80'}`}>HermesDeckX</button>
              <button type="button" onClick={() => setBackupMethod('hermesagent')} className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${backupMethod === 'hermesagent' ? 'bg-white dark:bg-white/10 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-white/60 hover:text-slate-700 dark:hover:text-white/80'}`}>HermesAgent</button>
            </div>
          </div>
          {backupMethod === 'hermesdeckx' && (<>
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-1 p-0.5 rounded-lg bg-slate-100 dark:bg-white/[0.05]">
                  {(['both', 'hermesagent', 'hermesdeckx'] as const).map(scope => (
                    <button key={scope} type="button" onClick={() => setManualSnapshotScope(scope)} className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${manualSnapshotScope === scope ? 'bg-white dark:bg-white/10 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-white/60 hover:text-slate-700 dark:hover:text-white/80'}`}>
                      {scope === 'both' ? (s.snapshotScopeBoth || 'Both') : scope === 'hermesagent' ? 'HermesAgent' : 'HermesDeckX'}
                    </button>
                  ))}
                </div>
        ...
        {/* Batch management toolbar + snapshot list — hide on config-history tab and when Hermes Agent backup method is active */}
        {snapshotModeTab !== 'config-history' && (snapshotModeTab !== 'manual' || backupMethod !== 'hermesagent') && <>
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-1.5">
            <select value={filterTrigger} onChange={e => setFilterTrigger(e.target.value)} className="px-2 py-1 rounded-lg text-[11px] border border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.04] text-slate-600 dark:text-white/60">
              ...
              <option value="import_hermesagent">HermesAgent</option>
          ...
          {filteredSnapshots.length === 0 ? (<div className="flex flex-col items-center py-10 text-slate-300 dark:text-white/10"><span className="material-symbols-outlined text-4xl mb-2">cloud_off</span><span className="text-[12px] text-slate-400 dark:text-white/20">{s.snapshotEmpty || s.noBackups}</span></div>) : (<>
            {filteredSnapshots.slice(0, snapshotListLimit).map((b) => { const id = b.id || b.snapshot_id; if (!id) return null; return (<div key={id} className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0"><div className="flex items-center gap-2"><p className="text-[13px] font-medium text-slate-700 dark:text-white/70 truncate">{b.note || id}</p>{b.trigger && b.trigger !== 'manual' && (<span className={`shrink-0 px-1.5 py-0.5 rounded text-[9px] font-medium leading-none ${b.trigger === 'scheduled' ? 'bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400' : b.trigger === 'pre_restore' ? 'bg-amber-100 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400' : b.trigger === 'import_hermesagent' ? 'bg-cyan-100 text-cyan-600 dark:bg-cyan-500/20 dark:text-cyan-400' : b.trigger === 'import' ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400' : 'bg-slate-100 text-slate-500 dark:bg-white/10 dark:text-white/50'}`}>{b.trigger === 'scheduled' ? (s.snapshotTriggerScheduled || 'Scheduled') : b.trigger === 'pre_restore' ? (s.snapshotTriggerPreRestore || 'Pre-restore') : b.trigger === 'import_hermesagent' ? 'HermesAgent' : b.trigger === 'import' ? (s.snapshotTriggerImport || 'Imported') : b.trigger}</span>)}</div><p className="text-[10px] text-slate-400 dark:text-white/20">{b.created_at ? new Date(b.created_at).toLocaleString() : ''} {b.size_bytes ? `· ${(b.size_bytes / 1024).toFixed(1)} KB` : ''}</p>
