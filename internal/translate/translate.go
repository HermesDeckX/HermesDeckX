...
// map from the hermes-agent config, or nil if unavailable.
...
// LLM config is auto-resolved from user's config.yaml on first use.
func New() *Translator {
	return &Translator{
...
// loadLocalProviders reads the providers map from config.yaml.
