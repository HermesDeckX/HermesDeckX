...
// hermesagentManifest mirrors the manifest.json structure from `hermesagent backup create`.
type hermesagentManifest struct {
...
// HermesAgentImportResult contains information about the imported backup.
type HermesAgentImportResult struct {
...
// ImportFromTarGz imports an HermesAgent `.tar.gz` backup archive, re-encrypts it
// as a HermesDeckX snapshot using the provided password, and stores it.
func (s *Service) ImportFromTarGz(data []byte, password, note string) (*HermesAgentImportResult, error) {
	if len(password) < 6 {
	...
	return &HermesAgentImportResult{
...
// parseTarGz extracts the HermesAgent manifest and all file contents from a tar.gz archive.
func parseTarGz(data []byte) (*hermesagentManifest, map[string][]byte, error) {
	...
	var manifest *hermesagentManifest
...
// convertHermesAgentToResources maps HermesAgent backup assets to HermesDeckX resource content.
func convertHermesAgentToResources(m *hermesagentManifest, files map[string][]byte) []ResourceContent {
	var resources []ResourceContent

	for archivePath, content := range files {
		logicalPath, resType, displayName := classifyHermesAgentAsset(archivePath, m)
		if logicalPath == "" {
			continue
		}

		restoreMode := RestoreModeFile
		if logicalPath == "files/config/config.yaml" {
...
// classifyHermesAgentAsset determines the logical path, resource type, and display name
// for a file extracted from an HermesAgent backup archive.
func classifyHermesAgentAsset(archivePath string, m *hermesagentManifest) (logicalPath, resType, displayName string) {
	...
	if lowerBase == "config.yaml" {
		return "files/config/config.yaml", "config", "config.yaml"
	}
	...
	if strings.Contains(stripped, ".hermes/") {
		parts := strings.SplitN(stripped, ".hermes/", 2)
		if len(parts) == 2 && parts[1] != "" {
			relPath := parts[1]
...
// ExportAsHermesAgentTarGz decrypts a snapshot and re-exports it as an HermesAgent-compatible
// .tar.gz archive with a manifest.json.
func (s *Service) ExportAsHermesAgentTarGz(snapshotID, password string) ([]byte, string, error) {
	...
	exportName := "hermesagent-backup-" + now.Format("2006-01-02_150405") + ".tar.gz"
...
func (s *Service) Stats() (*SnapshotStats, error) {
	...
	for _, r := range records {
		...
		case "import", "import_hermesagent":
			stats.ImportCount++
		}
	}
	if len(records) > 0 {
