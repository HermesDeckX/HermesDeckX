...
func (s *Service) SetGWClient(client *hermes.GWClient) {
	s.gwClient = client
}
...
func (s *Service) RestoreWithProgress(previewToken string, sel RestoreSelections, createPreRestore bool, passwordForPreRestore string, progressFn ProgressFn) (*RestoreResponse, error) {
	...
	if hasConfig {
		configData, ok := ub.Files["files/config/config.yaml"]
		if !ok {
			return nil, fmt.Errorf("backup missing hermes-agent config")
		}
		step++
		progressFn(RestoreProgressEvent{Phase: "config", Current: step, Total: totalSteps, File: "config.yaml"})
...
func (s *Service) writeResource(logicalPath string, data []byte) error {
	...
	// Last resort for config.yaml: CLI config set
	if logicalPath == "files/config/config.yaml" && hermes.IsHermesAgentInstalled() {
		current := map[string]any{}
		if err := json.Unmarshal(data, &current); err == nil {
			if err := hermes.ConfigApplyFull(current); err == nil {
				s.logStoreUse("write", "local", logicalPath)
				return nil
			}
		}
...
func scanGroupID(logicalPath, scope string) string {
	...
	if strings.HasPrefix(logicalPath, "files/hermesdeckx/") {
		return "hermesdeckx"
	}
	if strings.HasPrefix(logicalPath, "files/config/") {
		return "config"
	}
	if scope == BackupScopeHermesDeckX {
		return "hermesdeckx"
	}
	return "other"
}
...
func resolveLogicalPathDirect(logicalPath string) string {
	...
	// files/config/config.yaml → {stateDir}/config.yaml
	if logicalPath == "files/config/config.yaml" {
	...
	// files/hermesdeckx/{name} → {dataDir}/{name}
	if strings.HasPrefix(logicalPath, "files/hermesdeckx/") {
		rel := strings.TrimPrefix(logicalPath, "files/hermesdeckx/")
...
func (s *Service) restoreConfigPaths(snapshotConfig []byte, paths []string) error {
	...
	if s.isLocalGateway() {
		...
		if configPath == "" {
			configPath = filepath.Join(resolveStateDir(), "config.yaml")
		}
		...
		if b, ok, err := s.readResourceViaGateway("files/config/config.yaml"); ok {
			if err != nil {
				return err
			}
			_ = json.Unmarshal(b, &current)
	...
	return s.writeResource("files/config/config.yaml", out)
}
