...
func Doctor(args []string) int {
	...
	defaultPath := hermes.ResolveConfigPath()
	if defaultPath == "" {
		defaultPath = "~/.hermes/config.yaml"
	}
	...
	if *fixRuntime {
		changed, err := fixHermesAgentRuntimeNetworkInterfaces()
		if err != nil {
	...
	if *rollbackRuntimeFix {
		changed, err := rollbackHermesAgentRuntimeFix()
		if err != nil {
...
func runDoctorChecks(configPath string) doctorReport {
	...
	envIssues, envHasErrors := checkEnvConfig(expandPath("~/.hermes/env"))
	issues = append(issues, envIssues...)
	if envHasErrors {
		hasErrors = true
	}

	if _, err := os.Stat(filepath.Join(expandPath("~/.hermes"), "backups")); err != nil {
		issues = append(issues, doctorIssue{
			Level:      "info",
...
func runDoctorFixes(configPath string, report doctorReport) error {
	...
	if changed, err := fixEnvConfig(expandPath("~/.hermes/env")); err != nil {
		return err
	} else if changed {
...
func backupExistingConfig(path string) error {
	...
	dirs := []string{
		filepath.Join(expandPath("~/.hermes"), "backups"),
...
func checkEnvConfig(envPath string) ([]doctorIssue, bool) {
	...
	provider := strings.ToLower(strings.TrimSpace(values["HERMES_AGENT_AI_PROVIDER"]))
	model := strings.TrimSpace(values["HERMES_AGENT_AI_MODEL"])
	baseURL := strings.TrimSpace(values["HERMES_AGENT_BASE_URL"])
	apiKey := strings.TrimSpace(values["HERMES_AGENT_API_KEY"])
	if provider == "" || model == "" {
		issues = append(issues, doctorIssue{
			Level:      "error",
	...
	if strings.TrimSpace(values["HERMES_AGENT_BOT_NAME"]) == "" {
		issues = append(issues, doctorIssue{
			Level:      "warning",
	...
	if strings.TrimSpace(values["HERMES_AGENT_USER_NAME"]) == "" {
		issues = append(issues, doctorIssue{
			Level:      "warning",
	...
	if strings.TrimSpace(values["HERMES_AGENT_TIMEZONE"]) == "" {
		issues = append(issues, doctorIssue{
			Level:      "warning",
	...
	platform := strings.ToLower(strings.TrimSpace(values["HERMES_AGENT_NOTIFY_PLATFORM"]))
	switch platform {
	case "":
		issues = append(issues, doctorIssue{
			Level:      "warning",
	...
	case "custom":
		if strings.TrimSpace(firstNonEmpty(os.Getenv("HERMES_AGENT_NOTIFY_WEBHOOK"), values["HERMES_AGENT_NOTIFY_WEBHOOK"])) == "" {
			issues = append(issues, doctorIssue{
				Level:      "warning",
...
func fixEnvConfig(envPath string) (bool, error) {
	...
	if platform == "" {
		if strings.TrimSpace(values["TELEGRAM_BOT_TOKEN"]) != "" || strings.TrimSpace(values["TELEGRAM_CHAT_ID"]) != "" {
			values["HERMES_AGENT_NOTIFY_PLATFORM"] = "telegram"
			changed = true
		} else if strings.TrimSpace(values["SLACK_WEBHOOK_URL"]) != "" {
			values["HERMES_AGENT_NOTIFY_PLATFORM"] = "slack"
			changed = true
		} else if strings.TrimSpace(values["FEISHU_WEBHOOK_URL"]) != "" {
			values["HERMES_AGENT_NOTIFY_PLATFORM"] = "feishu"
			changed = true
		} else if strings.TrimSpace(values["HERMES_AGENT_NOTIFY_WEBHOOK"]) != "" {
			values["HERMES_AGENT_NOTIFY_PLATFORM"] = "custom"
			changed = true
		}
	}

	provider := strings.ToLower(strings.TrimSpace(values["HERMES_AGENT_AI_PROVIDER"]))
	if provider == "" && strings.TrimSpace(values["HERMES_AGENT_BASE_URL"]) != "" {
		values["HERMES_AGENT_AI_PROVIDER"] = "custom"
		changed = true
	}

	if strings.TrimSpace(values["HERMES_AGENT_TIMEZONE"]) == "" {
		if tz := strings.TrimSpace(os.Getenv("TZ")); tz != "" {
			values["HERMES_AGENT_TIMEZONE"] = tz
			changed = true
		}
	}
