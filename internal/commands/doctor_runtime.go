...
func fixHermesAgentRuntimeNetworkInterfaces() (bool, error) {
	root := expandPath("~/.npm-global/lib/node_modules/hermesagent/dist")
...
func patchRuntimeFile(path string) (bool, error) {
	...
	backup := fmt.Sprintf("%s.HermesDeckX-%s.bak", path, time.Now().Format("20060102-150405"))
...
func rollbackHermesAgentRuntimeFix() (bool, error) {
	root := expandPath("~/.npm-global/lib/node_modules/hermesagent/dist")
	pattern := filepath.Join(root, "gateway-cli-*.js")
...
func latestRuntimeBackup(target string) (string, bool, error) {
	pattern := target + ".HermesDeckX-*.bak"
