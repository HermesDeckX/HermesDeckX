const e="启用记忆搜索",n="配置向量搜索让 AI 助手能检索历史对话和知识文件，实现长期记忆",t={body:`记忆搜索（Memory Search）让 AI 助手可以通过语义检索查找历史对话和知识文件中的相关内容。这是实现「AI 不会忘事」的关键功能。
...
- hybrid.enabled — 启用混合搜索（向量 + 关键词），提高召回率`},{title:"保存并验证",description:"保存配置后，在聊天中提及之前对话过的内容，观察 AI 是否能回忆起来。首次启用可能需要等待索引构建完成（几分钟到十几分钟不等，取决于历史数据量）。"}]},i={name:e,description:n,content:t};export{t as content,i as default,n as description,e as name};
