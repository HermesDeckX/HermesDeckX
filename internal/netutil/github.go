...
// PyPI index mirrors for pip/uv install
var PipIndexMirrors = []MirrorSource{
	{Name: "PyPI Official", URL: "https://pypi.org/simple", Priority: 1},
	{Name: "Tsinghua (China)", URL: "https://pypi.tuna.tsinghua.edu.cn/simple", Priority: 2},
	{Name: "Aliyun (China)", URL: "https://mirrors.aliyun.com/pypi/simple", Priority: 3},
	{Name: "Tencent (China)", URL: "https://mirrors.cloud.tencent.com/pypi/simple", Priority: 4},
	{Name: "Huawei (China)", URL: "https://repo.huaweicloud.com/repository/pypi/simple", Priority: 5},
}
...
var (
	...
	pipIndexSelector      *MirrorSelector
)
...
func init() {
	...
	githubReleaseSelector = NewMirrorSelector(
		...
		"/HermesDeckX/HermesDeckX/releases",
		3*time.Second,
		24*time.Hour,
	)
	...
	pipIndexSelector = NewMirrorSelector(
		PipIndexMirrors,
		"/",
		3*time.Second,
		24*time.Hour,
...
// GetPipIndexURL returns the best PyPI index URL for pip/uv install
func GetPipIndexURL(ctx context.Context) string {
	best := pipIndexSelector.GetBest(ctx)
	return best.URL
}
...
func GetBestMirrorInfo(ctx context.Context) map[string]MirrorInfo {
	return map[string]MirrorInfo{
		...
		"pip_index":      getMirrorInfo(ctx, pipIndexSelector),
	}
}
...
func TestAllMirrors(ctx context.Context) map[string][]MirrorTestResult {
	return map[string][]MirrorTestResult{
		...
		"pip_index":      testMirrors(ctx, pipIndexSelector),
	}
}
...
func InvalidateAllCaches() {
	...
	pipIndexSelector.InvalidateCache()
}
