...
const (
	GitHubOwner = "HermesDeckX"
	GitHubRepo  = "HermesDeckX"
)
...
func CheckForUpdate(ctx context.Context) (*CheckResult, error) {
	...
	req.Header.Set("User-Agent", "HermesDeckX/"+currentVersion)
	...
	if err != nil {
		...
		if strings.Contains(errMsg, "timeout") || strings.Contains(errMsg, "connection refused") || strings.Contains(errMsg, "no route to host") {
			return &CheckResult{Available: false, CurrentVersion: currentVersion, Error: "Cannot connect to GitHub. Please check your network or download manually from https://github.com/HermesDeckX/HermesDeckX/releases"}, nil
		}
...
func ApplyUpdate(ctx context.Context, downloadURL string, progressFn func(ApplyProgress)) error {
	...
	tmpFile, err := os.CreateTemp(dir, "HermesDeckX-update-*.tmp")
	if err != nil {
...
func expectedAssetName() string {
	...
	return fmt.Sprintf("hermesdeckx-%s-%s%s", runtime.GOOS, runtime.GOARCH, ext)
}
