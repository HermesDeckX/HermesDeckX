...
// RefreshTokenFromConfig reloads the gateway auth token from the Hermes Agent config
...
// autoApprovePairing runs `hermes devices approve --latest` automatically
...
// autoRefreshToken attempts to reload the gateway auth token from the Hermes Agent
...
func readGatewayTokenFromConfig() string {
	// hermes-agent stores the gateway token in .env as HERMES_API_KEY
	// or in config.yaml under gateway.token
	...
	if data, err := os.ReadFile(envPath); err == nil {
		for _, line := range strings.Split(string(data), "\n") {
			...
			if strings.HasPrefix(line, "HERMES_API_KEY=") {
				v := strings.TrimPrefix(line, "HERMES_API_KEY=")
				v = strings.Trim(v, "\"'")
				if v != "" {
					return v
				}
	...
	// Fallback: try config.yaml gateway.token
	configPath := ResolveConfigPath()
	if configPath == "" {
		return ""
	}
	...
	for _, line := range strings.Split(string(data), "\n") {
		line = strings.TrimSpace(line)
		if strings.HasPrefix(line, "gateway_token:") || strings.HasPrefix(line, "api_token:") {
