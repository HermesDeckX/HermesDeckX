...
func DiagnoseGateway(host string, port int) *DiagnoseResult {
	...
	item := checkHermesAgentInstalled()
	result.Items = append(result.Items, item)
	if item.Status == DiagnoseFail {
		overallStatus = DiagnoseFail
	}

	configPath := hermesagentConfigPath()
...
func hermesagentConfigPath() string {
	return ResolveConfigPath()
}

func checkHermesAgentInstalled() DiagnoseItem {
	item := DiagnoseItem{
		Name:    "hermesagent_installed",
		Label:   i18n.T(i18n.MsgDiagnoseHermesAgentInstalled),
		LabelEn: "HermesAgent Installed",
	}

	cmdName := ResolveHermesAgentCmd()
	if cmdName == "" {
		item.Status = DiagnoseFail
		item.Detail = i18n.T(i18n.MsgDiagnoseHermesAgentNotDetected)
		item.Suggestion = i18n.T(i18n.MsgDiagnoseHermesAgentInstallSuggestion)
		return item
	}
	...
	if err == nil {
		...
		item.Detail = "hermesagent " + version
		return item
	}
...
func checkConfigValid(configPath string) DiagnoseItem {
	...
	// hermes-agent uses YAML config (config.yaml)
	var cfg map[string]interface{}
	if err := yaml.Unmarshal(data, &cfg); err != nil {
		item.Status = DiagnoseFail
		item.Detail = i18n.T(i18n.MsgDiagnoseYamlParseFailed, map[string]interface{}{"Error": err.Error()})
		item.Suggestion = i18n.T(i18n.MsgDiagnoseCheckYamlSyntax)
		return item
	}

	item.Status = DiagnosePass
	item.Detail = i18n.T(i18n.MsgDiagnoseValidYamlKeys, map[string]interface{}{"Count": len(cfg)})
	return item
}
...
func checkAuthToken(host string, port int, configPath string) DiagnoseItem {
	item := DiagnoseItem{
		...
		LabelEn: "API Key Configured",
	}

	// hermes-agent stores API keys in ~/.hermes/.env, not in config.yaml.
