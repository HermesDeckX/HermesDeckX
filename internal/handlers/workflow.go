...
func NewWorkflowHandler(client *hermes.GWClient) *WorkflowHandler {
...
func (h *WorkflowHandler) executeSequential(instance *WorkflowInstance, initialTask, prefix string) error {
	...
	for i, step := range instance.Definition.Steps {
		...
		// Spawn subagent using HermesAgent's sessions_spawn mechanism
...
// spawnSubagent spawns a subagent using HermesAgent's sessions_spawn mechanism.
func (h *WorkflowHandler) spawnSubagent(agentID, action, task string, timeoutSeconds int) (*SubagentResult, error) {
	...
	// Use HermesAgent's agent method to spawn a subagent
