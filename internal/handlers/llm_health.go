...
func NewLLMHealthHandler(svc *hermes.Service) *LLMHealthHandler {
	return &LLMHealthHandler{svc: svc}
}

func (h *LLMHealthHandler) SetGWClient(client *hermes.GWClient) {
	h.gwClient = client
}
...
func (h *LLMHealthHandler) ModelsStatus(w http.ResponseWriter, r *http.Request) {
	...
	out, err := hermes.RunCLI(ctx, "models", "status", "--json")
	if err != nil {
		...
		web.Fail(w, r, "CLI_ERROR", fmt.Sprintf("hermesagent models status failed: %v", err), http.StatusBadGateway)
		return
	}
...
func (h *LLMHealthHandler) ExecCapability(w http.ResponseWriter, r *http.Request) {
	...
	hasLocal := hermes.IsHermesAgentInstalled()
...
// Exec runs an hermes CLI command and returns its output.
...
func (h *LLMHealthHandler) Exec(w http.ResponseWriter, r *http.Request) {
	...
	// Security: only allow hermes CLI commands
	allowedCommands := map[string]bool{
		"hermes": true,
	}
	baseCmd := strings.TrimSpace(req.Command)
	if !allowedCommands[baseCmd] {
		web.Fail(w, r, "FORBIDDEN", "only hermes commands are allowed", http.StatusForbidden)
		return
	}
...
func (h *LLMHealthHandler) execLocal(w http.ResponseWriter, r *http.Request, req cliExecRequest) {
	...
	if cmdPath == "" {
		web.Fail(w, r, "NOT_INSTALLED", "hermesagent command not found", http.StatusBadGateway)
		return
	}
...
var exactReadonlyCLIArgs = [][]string{
	// JSON-output commands (remote RPC mapping targets)
	{"--version"},
	...
	// Plain-text hermes CLI commands (Test Center presets)
	{"doctor"},
	{"status"},
	{"gateway", "status"},
	{"gateway", "status", "--deep"},
...
func (h *LLMHealthHandler) parseModelsStatusOutput(raw string) *llmModelsStatusResponse {
	...
	// Try to parse the JSON output from `hermesagent models status --json`
...
func (h *LLMHealthHandler) parseProbeOutput(raw string, totalMs int64) *llmProbeResponse {
	...
	// Try to parse the JSON output from `hermesagent models status --probe --json`
	var parsed map[string]json.RawMessage
	if err := json.Unmarshal([]byte(raw), &parsed); err != nil {
		idx := strings.Index(raw, "{")
		if idx < 0 {
