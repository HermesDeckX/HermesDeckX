...
type MirrorConfig struct {
	...
	HermesHubRegistry string `json:"hermesHubRegistry"` // HermesHub mirror URL, "" = use default
}
...
const (
	...
	settingKeyMirrorHermesHub = "mirror_hermeshub_registry"
)
...
func (h *MirrorConfigHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	cfg := MirrorConfig{
		...
		HermesHubRegistry: strVal(all, settingKeyMirrorHermesHub, ""),
	}
	web.OK(w, r, cfg)
}
...
func (h *MirrorConfigHandler) Set(w http.ResponseWriter, r *http.Request) {
	...
	err := h.settingRepo.SetBatch(map[string]string{
		...
		settingKeyMirrorHermesHub: cfg.HermesHubRegistry,
	})
	if err != nil {
...
func (h *MirrorConfigHandler) ApplyToSystem(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Tools []string     `json:"tools"` // which tools to apply: "npm","go","pip","git","docker","hermeshub"
	...
	for _, tool := range req.Tools {
		...
		case "hermeshub":
			ok, msg = applyHermesHubRegistry(req.Cfg.HermesHubRegistry)
		default:
...
func applyHermesHubRegistry(registry string) (bool, string) {
	...
	if url == "" {
		// Reset to default
		url = webconfig.Default().Server.HermesHubQueryURL
	}
	cfg.Server.HermesHubQueryURL = url
	if err := webconfig.Save(cfg); err != nil {
		return false, fmt.Sprintf("save webconfig failed: %v", err)
	}
	return true, fmt.Sprintf("HermesHub registry set to %s", url)
}
