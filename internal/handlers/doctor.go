...
func NewDoctorHandler(svc *hermes.Service) *DoctorHandler {
...
func (h *DoctorHandler) SetGWClient(client *hermes.GWClient) {
	h.gwClient = client
}
...
func securityPermissionFixPath(checkID string) string {
	...
	configPath := hermes.ResolveConfigPath()
	switch checkID {
	case "fs.state_dir.perms_world_writable", "fs.state_dir.perms_group_writable", "fs.state_dir.perms_readable":
		return stateDir
	case "fs.config.perms_writable", "fs.config.perms_world_readable", "fs.config.perms_group_readable":
...
func normalizeCheckKey(item CheckItem) string {
	...
	switch {
	case strings.Contains(id, "hermesagent_installed") || strings.Contains(id, "hermes.install"):
		return "check:hermesagent_install"
...
func (h *DoctorHandler) Run(w http.ResponseWriter, r *http.Request) {
	...
	items = append(items, h.checkHermesAgentService())
	items = append(items, h.checkHermesDeckXService())
...
func (h *DoctorHandler) Overview(w http.ResponseWriter, r *http.Request) {
	...
	diag := hermes.DiagnoseGateway(h.svc.GatewayHost, h.svc.GatewayPort)
	availability := 100.0
	if diag != nil {
		pass := 0
		total := len(diag.Items)
		for _, it := range diag.Items {
			if it.Status == hermes.DiagnosePass {
				pass++
			}
		}
		if total > 0 {
...
func (h *DoctorHandler) Fix(w http.ResponseWriter, r *http.Request) {
	...
	for _, res := range results {
		if res.Status == "success" && strings.HasPrefix(res.ID, "security.") {
			hermes.InvalidateSecurityAuditCache()
			break
		}
	}
...
// CLIFix runs "hermes doctor --fix" on the local machine.
// This does not require a gateway WS connection — only a local hermes CLI install.
func (h *DoctorHandler) CLIFix(w http.ResponseWriter, r *http.Request) {
	cmdPath := hermes.ResolveHermesAgentCmd()
	if cmdPath == "" {
		web.Fail(w, r, "CLI_NOT_INSTALLED", "hermes CLI is not installed on this machine", http.StatusUnprocessableEntity)
		return
	}
	...
	// hermes doctor only supports --fix (no --yes or --non-interactive)
	cmd := exec.CommandContext(ctx, cmdPath, "doctor", "--fix")
	...
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			...
			web.Fail(w, r, "CLI_FIX_TIMEOUT", "hermesagent doctor --fix timed out after 60s", http.StatusGatewayTimeout)
			return
		} else {
	...
	logger.Doctor.Info().Int("exitCode", exitCode).Msg("hermesagent doctor --fix completed")
	web.OK(w, r, map[string]interface{}{
		"exitCode": exitCode,
...
func (h *DoctorHandler) checkInstalled() CheckItem {
	...
	if cmdPath != "" {
		return CheckItem{
			ID:       "hermes.install",
			Code:     "hermes.install",
			Name:     "HermesAgent Install",
			Category: "runtime",
			Severity: "info",
			Status:   "ok",
			Detail:   "installed: " + cmdPath,
		}
	}
	return CheckItem{
		ID:         "hermes.install",
		...
		Severity:   "error",
		Status:     "error",
		Detail:     "hermesagent command not found",
		Suggestion: "install HermesAgent first",
	}
}

func (h *DoctorHandler) checkConfig() CheckItem {
	if hermes.ConfigFileExists() {
		path := hermes.ResolveConfigPath()
		info, _ := os.Stat(path)
		if info != nil {
...
func (h *DoctorHandler) checkPIDLock() CheckItem {
	pidFile := filepath.Join(hermes.ResolveStateDir(), "gateway.pid")
...
func (h *DoctorHandler) checkHermesAgentService() CheckItem {
	...
	if status.Installed {
		...
		return CheckItem{
			ID:       "service.hermesagent",
			Code:     "service.hermesagent",
			Name:     "HermesAgent Service",
			Category: "service",
			Severity: "info",
			Status:   "ok",
			Detail:   detail,
...
func (h *DoctorHandler) checkHermesDeckXService() CheckItem {
	if service.IsInstalled() {
		return CheckItem{
			ID:       "service.hermesdeckx",
			Code:     "service.hermesdeckx",
			Name:     "HermesDeckX Service",
			Category: "service",
			Severity: "info",
			Status:   "ok",
	...
	return CheckItem{
		...
		Severity:   "info",
		Status:     "warn",
		Detail:     "system service not installed, HermesDeckX requires manual start",
		Suggestion: "install the system service in Settings → Update → System Service for auto-start on boot",
	}
}
...
func (h *DoctorHandler) collectChecks() []CheckItem {
	items := []CheckItem{
		...
		h.checkPort(),
		h.checkDisk(),
		h.checkHermesAgentService(),
		h.checkHermesDeckXService(),
	}
...
// checkConfigValues inspects config.yaml for common misconfigurations
// and returns fixable CheckItems for each issue found.
// hermes-agent uses YAML config with top-level keys: model, providers, gateway, toolsets, etc.
func (h *DoctorHandler) checkConfigValues() []CheckItem {
	...
	if model == "" || model == "<nil>" {
		items = append(items, CheckItem{
			...
			Severity:   "warn",
			Status:     "warn",
			Detail:     "model is not set in config.yaml",
			Suggestion: "set a default model in config.yaml (e.g. model: anthropic/claude-sonnet-4-20250514)",
			Fixable:    true,
		})
	} else {
		items = append(items, CheckItem{
	...
	// Check 2: API key configured in .env
	...
	if envPath != "" {
		if envData, err := os.ReadFile(envPath); err == nil {
			...
			apiKeyPatterns := []string{
				...
				"DEEPSEEK_API_KEY=", "NOUS_API_KEY=", "GLM_API_KEY=",
	...
	if !hasAPIKey {
		items = append(items, CheckItem{
			...
			Severity:   "warn",
			Status:     "warn",
			Detail:     "no API key found in .env file",
			Suggestion: "run 'hermes-agent setup' to configure API keys",
		})
	} else {
		items = append(items, CheckItem{
			ID:       "config.api_key",
			...
			Severity: "info",
			Status:   "ok",
			Detail:   "API key configured in .env",
		})
	}

	// Check 3: gateway port (optional, hermes-agent defaults to 8642)
	gw, _ := raw["gateway"].(map[string]any)
	if gw != nil {
		if port := gw["port"]; port != nil {
			items = append(items, CheckItem{
				ID:       "config.gateway_port",
				Code:     "config.gateway_port",
				Name:     "Gateway Port",
				Category: "config",
				Severity: "info",
				Status:   "ok",
				Detail:   fmt.Sprintf("gateway.port: %v", port),
			})
		}
	}

	// Check 4: backup directory
	backupDir := filepath.Join(hermes.ResolveStateDir(), "backups")
...
func (h *DoctorHandler) gatewayDiagnoseChecks() []CheckItem {
	...
	for _, it := range diag.Items {
		...
		severity := "info"
		switch it.Status {
		case hermes.DiagnoseFail:
			status = "error"
			severity = "error"
		case hermes.DiagnoseWarn:
			status = "warn"
			severity = "warn"
		}
...
func (h *DoctorHandler) runFix(item CheckItem) fixItemResult {
	...
	case "config.model":
		// hermes-agent: set a default model if missing
		return h.fixConfigKey("", "model", "anthropic/claude-sonnet-4-20250514", "set default model to anthropic/claude-sonnet-4-20250514")
...
// fixConfigKey reads config.yaml (YAML), sets a key in a section, backs up, and writes.
func (h *DoctorHandler) fixConfigKey(section, key, value, msg string) fixItemResult {
	...
	// For top-level keys (section == ""), set directly
	if section == "" {
		existing := strings.TrimSpace(fmt.Sprint(raw[key]))
		if existing != "" && existing != "<nil>" {
			res.Status = "skipped"
			res.Message = key + " already set to " + existing
			return res
		}
		raw[key] = value
	} else {
...
// fixConfigAuthToken sets gateway.token in config.yaml.
// hermes-agent uses gateway.token (not gateway.auth.token like OpenClaw).
func (h *DoctorHandler) fixConfigAuthToken(token string) fixItemResult {
	...
	res.Status = "success"
	res.Message = "generated and configured gateway token"
	return res
}
...
// backupAndWriteConfig creates a timestamped backup of the config and writes the updated config as YAML.
func backupAndWriteConfig(configPath string, raw map[string]any) error {
	...
	if err == nil && len(existingData) > 0 {
		...
		backupPath := filepath.Join(backupDir, fmt.Sprintf("config.yaml.%s.bak", time.Now().Format("20060102-150405")))
		_ = os.WriteFile(backupPath, existingData, 0o600)
	}

	// hermes-agent uses YAML config format
...
func (h *DoctorHandler) securityAuditChecks() []CheckItem {
	report, err := hermes.RunSecurityAuditCached(h.gwClient)
	...
	for _, f := range report.Findings {
		...
		if suggestion == "" && f.Severity == "critical" {
			suggestion = "Run: hermesagent security audit --deep"
		}
...
func (h *DoctorHandler) SecurityScanAndAlert() {
	report, err := hermes.RunSecurityAuditWithGW(h.gwClient)
	if err != nil || report == nil {
		return
	}
	for _, f := range report.Findings {
