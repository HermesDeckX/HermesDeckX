...
if [ -z "$SELF_SCRIPT" ] || [ "$SELF_SCRIPT" = "bash" ] || [ "$SELF_SCRIPT" = "/bin/bash" ] || [ ! -f "$SELF_SCRIPT" ]; then
    SELF_SCRIPT="/tmp/.hermesdeckx-installer.sh"
    # When piped, save stdin (already consumed) — re-download if needed
    if [ ! -f "$SELF_SCRIPT" ]; then
        curl -fsSL "https://raw.githubusercontent.com/HermesDeckX/HermesDeckX/main/install.sh" -o "$SELF_SCRIPT" 2>/dev/null || true
    fi
fi
...
BINARY_NAME="hermesdeckx"
DEFAULT_PORT=19788
INTERNAL_PORT=19788
DEFAULT_HOST_PORT=19700
PORT=$DEFAULT_PORT
# Config and data directories are relative to executable location
CONFIG_DIR=""
DATA_DIR=""

# Function to check if HermesDeckX is installed and find its location
check_installed() {
    ...
    if [ -f "./$BINARY_NAME" ] && [ -x "./$BINARY_NAME" ]; then
        ...
        # Extract version number from output like "HermesDeckX 0.0.11"
...
# Priority: OHD_PORT env > data/HermesDeckX.json server.port > DEFAULT_PORT
get_config_port() {
    # Check environment variable first
    if [ -n "$OHD_PORT" ] && [ "$OHD_PORT" -gt 0 ] 2>/dev/null && [ "$OHD_PORT" -le 65535 ] 2>/dev/null; then
        PORT=$OHD_PORT
        return
    fi
    ...
    if [ -n "$INSTALLED_LOCATION" ]; then
        config_file="$(dirname "$INSTALLED_LOCATION")/data/HermesDeckX.json"
    elif [ -n "$INSTALLED_BINARY" ]; then
        config_file="$(dirname "$INSTALLED_BINARY")/data/HermesDeckX.json"
    else
        config_file="$(pwd)/data/HermesDeckX.json"
    fi
...
print_access_urls() {
    local port="${1:-$PORT}"
    echo -e "${CYAN}Access HermesDeckX at / 访问 HermesDeckX：${NC}"
...
check_systemd_service() {
    ...
    USER_SERVICE_PATH="$HOME/.config/systemd/user/hermesdeckx.service"
    ...
    SYSTEM_SERVICE_PATH="/etc/systemd/system/hermesdeckx.service"
    ...
    if systemctl --user is-enabled --quiet hermesdeckx 2>/dev/null; then
        SYSTEMD_SERVICE_INSTALLED=true
        SYSTEMD_SERVICE_TYPE="user"
        return 0
    fi
    
    if systemctl is-enabled --quiet hermesdeckx 2>/dev/null; then
        SYSTEMD_SERVICE_INSTALLED=true
        SYSTEMD_SERVICE_TYPE="system"
        return 0
    fi
...
[Unit]
Description=HermesDeckX Service
...
EOF
    ...
    if systemctl --user daemon-reload 2>/dev/null; then
        systemctl --user enable hermesdeckx 2>/dev/null
        ...
        echo -e "  ${GREEN}systemctl --user enable hermesdeckx${NC}"
...
stop_service() {
    ...
    if check_systemd_service; then
        ...
        if [ "$SYSTEMD_SERVICE_TYPE" = "user" ]; then
            systemctl --user stop hermesdeckx
            echo -e "${GREEN}✓ Service stopped / 服务已停止${NC}"
        else
            sudo systemctl stop hermesdeckx
            echo -e "${GREEN}✓ Service stopped / 服务已停止${NC}"
        fi
        
        echo ""
        echo -e "${CYAN}You can now start HermesDeckX manually with: / 现在可以手动启动 HermesDeckX：${NC}"
...
DOCKER_COMPOSE_FILE="docker-compose-hermesdeckx.yml"
NEED_MIRROR=false
DOCKER_MIRROR=""
...
# Register HermesDeckX binary in PATH via ~/.local/bin symlink (preferred)
...
register_path() {
    ...
    if mkdir -p "$link_dir" 2>/dev/null; then
        ln -sf "$binary_abs" "$link_dir/hermesdeckx"
        echo -e "${GREEN}✓ Created symlink: $link_dir/hermesdeckx → $binary_abs${NC}"
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -qx "$link_dir"; then
            ...
            echo -e "${YELLOW}  Run ${GREEN}source ~/${profile_file##*/}${YELLOW} or open a new terminal to use ${GREEN}hermesdeckx${YELLOW} globally${NC}"
        else
...
# Remove HermesDeckX PATH registration (for uninstall)
# Usage: unregister_path
unregister_path() {
    local link_dir="$HOME/.local/bin"

    # Remove symlink
    if [ -L "$link_dir/hermesdeckx" ]; then
        rm -f "$link_dir/hermesdeckx"
        echo -e "${GREEN}✓ Removed symlink: $link_dir/hermesdeckx${NC}"
    fi
...
docker_install_new() {
    ...
    if [ -f "docker-compose-hermesdeckx.yml" ] || { [ -f "docker-compose.yml" ] && grep -q "knowhunters/hermesdeckx" "docker-compose.yml" 2>/dev/null; }; then
        # Default instance exists, find next available number
        local n=2
        while [ -f "docker-compose-hermesdeckx-${n}.yml" ]; do
            n=$((n + 1))
        done
        default_name="hermesdeckx-${n}"
    fi
    ...
    echo -e "Examples: hermesdeckx-2, hermesdeckx-dev, hermesdeckx-test"
    echo ""
    ...
    local check_file="docker-compose-hermesdeckx.yml"
    if [ "$INSTANCE_NAME" != "hermesdeckx" ]; then
...
# Install HermesDeckX via Docker
# Usage: docker_install [instance_name]
#   instance_name: unique name for this Docker deployment (default: hermesdeckx)
#   When not "hermesdeckx", compose file is saved as docker-compose-{name}.yml
#   with unique container name and volume names for isolation.
docker_install() {
    local instance_name="${1:-hermesdeckx}"
    local compose_file="docker-compose-hermesdeckx.yml"
    # Backward compat: if old docker-compose.yml has our image, use it
    if [ "$instance_name" = "hermesdeckx" ] && [ ! -f "docker-compose-hermesdeckx.yml" ] && [ -f "docker-compose.yml" ] && grep -q "knowhunters/hermesdeckx" "docker-compose.yml" 2>/dev/null; then
        compose_file="docker-compose.yml"
    elif [ "$instance_name" != "hermesdeckx" ]; then
        compose_file="docker-compose-${instance_name}.yml"
    fi

    echo ""
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}  Install HermesDeckX (Docker) / 安装 HermesDeckX (Docker)${NC}"
    if [ "$instance_name" != "hermesdeckx" ]; then
    ...
    if [ "$instance_name" != "hermesdeckx" ]; then
        ...
        sed_inplace "s/container_name: hermesdeckx/container_name: ${instance_name}/" "$compose_file"
        sed_inplace "s/name: hermesdeckx-data/name: ${instance_name}-data/" "$compose_file"
        sed_inplace "s/name: hermesagent-data/name: ${instance_name}-hermesagent-data/" "$compose_file"
        sed_inplace "s/name: hermesdeckx-runtime/name: ${instance_name}-runtime/" "$compose_file"
        sed_inplace "s/name: hermesdeckx-net/name: ${instance_name}-net/" "$compose_file"
        echo -e "${GREEN}✓ Configured for instance: $instance_name${NC}"
    fi

    # Step 4: Smart port configuration — auto-detect available host port
    # Docker maps HOST_PORT → INTERNAL_PORT (19788). Host port starts from DEFAULT_HOST_PORT (19700).
    ...
    for _cf in docker-compose-hermesdeckx.yml docker-compose.yml docker-compose-*.yml; do
        [ -f "$_cf" ] || continue
        [ "$_cf" = "$compose_file" ] && continue  # Skip our own file
        local _ap
        _ap=$(grep -oE '"[0-9]+:19788"' "$_cf" 2>/dev/null | head -1 | grep -oE '^"[0-9]+' | tr -d '"')
    ...
    # Inject OHD_HOST_PORT so the container banner shows the correct external URL
    if grep -q "OHD_HOST_PORT" "$compose_file" 2>/dev/null; then
        sed_inplace "s/OHD_HOST_PORT: .*/OHD_HOST_PORT: \"${PORT}\"/" "$compose_file"
    else
        sed_inplace "/OHD_HERMES_AGENT_GATEWAY_PORT:/a\\      OHD_HOST_PORT: \"${PORT}\"" "$compose_file"
    fi
    ...
    echo ""
    echo -e "${BLUE}Starting HermesDeckX container... / 正在启动 HermesDeckX 容器...${NC}"
    $compose_run up -d

    # Step 6: Wait for health check
    # The entrypoint waits up to 120s for the gateway on first boot, then starts
    # HermesDeckX. We need to wait longer than 120s to allow the full startup chain.
    echo ""
    echo -e "${CYAN}Waiting for HermesDeckX to become ready (first boot may take ~2 min)...${NC}"
    echo -e "${CYAN}等待 HermesDeckX 就绪（首次启动可能需要约 2 分钟）...${NC}"
    local max_wait=150
    local waited=0
    ...
    if [ $waited -ge $max_wait ]; then
        echo -e "${YELLOW}⚠ HermesDeckX is still starting. Check status with:"
        echo -e "  HermesDeckX 仍在启动中，请用以下命令检查状态：${NC}"
        echo -e "  ${GREEN}$compose_run ps${NC}"
        echo -e "  ${GREEN}$compose_run logs --tail 30${NC}"
    else
        echo -e "${GREEN}✓ HermesDeckX is ready! / HermesDeckX 已就绪！${NC}"
    fi
    ...
    local vol_data vol_hermesagent vol_runtime
    vol_data=$(grep -A1 'hermesdeckx-data:' "$compose_file" 2>/dev/null | grep 'name:' | awk '{print $2}' || echo "${instance_name}-data")
    vol_hermesagent=$(grep -A1 'hermesdeckx-hermesagent-data:' "$compose_file" 2>/dev/null | grep 'name:' | awk '{print $2}' || echo "${instance_name}-hermesagent-data")
    vol_runtime=$(grep -A1 'hermesdeckx-runtime:' "$compose_file" 2>/dev/null | grep 'name:' | awk '{print $2}' || echo "${instance_name}-runtime")
    local vol_base="/var/lib/docker/volumes"
    echo -e "${CYAN}📂 Data volumes (host path) / 数据卷（宿主机路径）：${NC}"
    echo -e "  HermesDeckX:  ${GREEN}${vol_base}/${vol_data}/_data${NC}"
    echo -e "  HermesAgent:   ${GREEN}${vol_base}/${vol_hermesagent}/_data${NC}"
    echo -e "  Runtime:    ${GREEN}${vol_base}/${vol_runtime}/_data${NC}"
    echo ""
    echo -e "${YELLOW}🔐 Initial Login Credentials / 初始登录凭据：${NC}"
    echo ""
    local _cred
    _cred=$(docker exec "$instance_name" cat /data/hermesdeckx/bootstrap/credentials 2>/dev/null || true)
    if [ -n "$_cred" ]; then
        local _user _pass
...
# Update HermesDeckX Docker deployment
# Usage: docker_update <compose_file> <instance_name>
docker_update() {
    local compose_file="${1:-$DOCKER_COMPOSE_FILE}"
    local instance_name="${2:-hermesdeckx}"
    local compose_run="$COMPOSE_CMD -f $compose_file -p $instance_name"

    echo ""
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}  Update HermesDeckX (Docker) / 更新 HermesDeckX (Docker)${NC}"
    if [ "$instance_name" != "hermesdeckx" ]; then
        echo -e "${YELLOW}  Instance / 实例: ${instance_name}${NC}"
    fi
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    ...
    local compose_port
    compose_port=$(grep -oE '"[0-9]+:19788"' "$compose_file" 2>/dev/null | head -1 | grep -oE '^"[0-9]+' | tr -d '"' || true)
    ...
    echo ""
    echo -e "${CYAN}Waiting for HermesDeckX to become ready... / 等待 HermesDeckX 就绪...${NC}"
    local max_wait=150
    local waited=0
    while [ $waited -lt $max_wait ]; do
        if curl -sf "http://localhost:${PORT}/api/v1/health" >/dev/null 2>&1; then
...
# Uninstall HermesDeckX Docker deployment
...
docker_uninstall() {
    ...
    echo ""
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}  Uninstall HermesDeckX (Docker: ${instance_name}) / 卸载 HermesDeckX (Docker: ${instance_name})${NC}"
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo ""

    echo -e "${CYAN}This will: / 将执行：${NC}"
    echo "  - Stop and remove the HermesDeckX container / 停止并删除 HermesDeckX 容器"
    echo ""
...
docker_management_menu() {
    ...
    local compose_port
    compose_port=$(grep -oE '"[0-9]+:19788"' "$compose_file" 2>/dev/null | head -1 | grep -oE '^"[0-9]+' | tr -d '"')
    if [ -n "$compose_port" ]; then
        PORT=$compose_port
    fi
    ...
    case $CHOICE in
        ...
        2)
            if [ "$is_running" = true ]; then
                echo ""
                echo -e "${BLUE}Stopping HermesDeckX container... / 正在停止 HermesDeckX 容器...${NC}"
                $compose_run stop
        ...
        3)
            echo ""
            echo -e "${BLUE}Restarting HermesDeckX container... / 正在重启 HermesDeckX 容器...${NC}"
            $compose_run restart
        ...
        5)
            ...
            echo ""
            if docker ps --filter "name=^${container_name}$" --filter "status=running" --format '{{.Names}}' 2>/dev/null | grep -q .; then
                echo -e "${GREEN}✓ HermesDeckX is running / HermesDeckX 运行中${NC}"
                print_access_urls "$PORT"
            else
                echo -e "${YELLOW}HermesDeckX is not running / HermesDeckX 未运行${NC}"
            fi
            ;;
        6)
...
echo -e "${CYAN}:: HermesDeckX Launcher - ${LATEST_VERSION} ::${NC}"
echo ""
...
if [ "$IS_CONTAINER" = false ] && check_docker && check_docker_compose; then
    ...
    for cf in docker-compose-hermesdeckx.yml docker-compose.yml docker-compose-*.yml; do
        [ -f "$cf" ] || continue
        if grep -q "knowhunters/hermesdeckx" "$cf" 2>/dev/null; then
            ...
            if [ "$cf" = "docker-compose-hermesdeckx.yml" ] || [ "$cf" = "docker-compose.yml" ]; then
                DOCKER_INSTANCE_NAMES+=("hermesdeckx")
            else
            ...
            # Extract port
            local_port=$(grep -oE '"[0-9]+:19788"' "$cf" 2>/dev/null | head -1 | grep -oE '^"[0-9]+' | tr -d '"')
            ...
            local_container="${local_container:-hermesdeckx}"
            # Version
...
# Function to uninstall HermesDeckX
uninstall() {
    echo ""
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}  Uninstall HermesDeckX / 卸载 HermesDeckX${NC}"
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    if [ -n "$INSTALLED_LOCATION" ]; then
        ...
        if [ "$MODE" = "1" ]; then
            ...
            echo "  - $INSTALLED_LOCATION (HermesDeckX)"
            
            SYSTEMD_UNINSTALL=false
            if check_systemd_service; then
                echo "  - hermesdeckx.service (systemd service / systemd 服务)"
            ...
            if check_systemd_service; then
                ...
                echo "  - hermesdeckx.service ($SYSTEMD_SERVICE_TYPE-level)"
            ...
            echo "  - HermesDeckX：${RED}will be removed / 将被删除${NC}"
        ...
        echo -e "${RED}HermesDeckX is not installed / HermesDeckX 未安装${NC}"
        exit 1
    fi
}
...
perform_uninstall() {
    ...
    if [ "$SYSTEMD_UNINSTALL" = true ]; then
        ...
        if [ "$SYSTEMD_SERVICE_TYPE" = "user" ]; then
            ...
            systemctl --user stop hermesdeckx 2>/dev/null || true
            echo -e "${YELLOW}Disabling service... / 正在禁用服务...${NC}"
            systemctl --user disable hermesdeckx 2>/dev/null || true
            ...
            sudo systemctl stop hermesdeckx 2>/dev/null || true
            echo -e "${YELLOW}Disabling service... / 正在禁用服务...${NC}"
            sudo systemctl disable hermesdeckx 2>/dev/null || true
    ...
    echo -e "${GREEN}✓ Removed HermesDeckX / 已删除 HermesDeckX${NC}"
...
# Function to check if HermesDeckX is running
check_process_running() {
    ...
    if check_systemd_service; then
        if [ "$SYSTEMD_SERVICE_TYPE" = "user" ]; then
            if systemctl --user is-active --quiet hermesdeckx 2>/dev/null; then
                return 0
            fi
        else
            if systemctl is-active --quiet hermesdeckx 2>/dev/null; then
                return 0
            fi
        fi
    fi
...
# Function to stop HermesDeckX
stop_hermesdeckx() {
    ensure_user_systemd_env
    echo -e "${CYAN}Stopping HermesDeckX... / 正在停止 HermesDeckX...${NC}"
    ...
    if check_systemd_service; then
        if [ "$SYSTEMD_SERVICE_TYPE" = "user" ] && ! can_use_systemctl_user; then
            ...
            if [ "$SYSTEMD_SERVICE_TYPE" = "user" ]; then
                systemctl --user stop hermesdeckx > /dev/null 2>&1 || true
            else
                sudo systemctl stop hermesdeckx > /dev/null 2>&1 || true
            fi
            sleep 2
        fi
    fi
    ...
    echo -e "${GREEN}✓ HermesDeckX stopped / HermesDeckX 已停止${NC}"
}

# Function to start HermesDeckX
start_hermesdeckx() {
    ensure_user_systemd_env
    echo -e "${CYAN}Starting HermesDeckX... / 正在启动 HermesDeckX...${NC}"
    ...
    if check_systemd_service; then
        if [ "$SYSTEMD_SERVICE_TYPE" = "user" ] && ! can_use_systemctl_user; then
            ...
            if [ "$SYSTEMD_SERVICE_TYPE" = "user" ]; then
                systemctl --user start hermesdeckx > /dev/null 2>&1
            else
                sudo systemctl start hermesdeckx > /dev/null 2>&1
            fi
            sleep 2
    ...
    local err_file="/tmp/.hermesdeckx-start-err.log"
    echo -e "${BLUE}Starting HermesDeckX on port $PORT... / 正在启动 HermesDeckX (端口 $PORT)...${NC}"
    ...
    if pgrep -f "$BINARY_NAME" > /dev/null 2>&1; then
        echo -e "${GREEN}✓ HermesDeckX started successfully / HermesDeckX 启动成功${NC}"
        rm -f "$err_file"
        return 0
    else
        echo -e "${YELLOW}⚠ Failed to start HermesDeckX / 启动 HermesDeckX 失败${NC}"
        if [ -s "$err_file" ]; then
...
# Function to update HermesDeckX
update() {
    echo ""
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}  Update HermesDeckX / 更新 HermesDeckX${NC}"
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    ASSET_PATTERN="hermesdeckx-${OS}-${ARCH}"
    DOWNLOAD_URL=""
    ...
    # Check if HermesDeckX is currently running
    echo ""
    if check_process_running; then
        echo -e "${YELLOW}⚠ HermesDeckX is currently running / HermesDeckX 正在运行${NC}"
        echo -e "${YELLOW}The program needs to be stopped before updating. / 更新前需要停止程序。${NC}"
        echo ""
        echo -n "Stop HermesDeckX now and continue update? / 立即停止 HermesDeckX 并继续更新？ [Y/n] "
        read -n 1 -r </dev/tty
        echo
        if [[ $REPLY =~ ^[Nn]$ ]]; then
            echo -e "${YELLOW}Update cancelled. / 更新已取消${NC}"
    ...
    # Ask if user wants to restart HermesDeckX
    echo -n "Start HermesDeckX now? / 立即启动 HermesDeckX？ [Y/n] "
    read -n 1 -r </dev/tty
    echo
    ...
    if [[ ! $REPLY =~ ^[Nn]$ ]]; then
        ...
        if check_process_running; then
            ...
            if check_systemd_service; then
                ...
                echo -e "  ${GREEN}systemctl --user status hermesdeckx${NC}   - Check status / 查看状态"
                echo -e "  ${GREEN}systemctl --user stop hermesdeckx${NC}     - Stop service / 停止服务"
            fi
        fi
    else
        ...
        if check_systemd_service; then
            ...
            echo -e "  ${GREEN}systemctl --user start hermesdeckx${NC}    - Start service / 启动服务"
            echo -e "  ${GREEN}systemctl --user stop hermesdeckx${NC}     - Stop service / 停止服务"
            echo -e "  ${GREEN}systemctl --user status hermesdeckx${NC}   - Check status / 查看状态"
        fi
    fi
...
if [ "$HAS_DOCKER" = true ] || [ "$HAS_BINARY" = true ]; then
    ...
    if [ "$HAS_DOCKER" = true ]; then
        for i in "${!DOCKER_INSTANCES[@]}"; do
            ...
            local_label="Docker"
            if [ ${#DOCKER_INSTANCES[@]} -gt 1 ] || [ "$local_name" != "hermesdeckx" ]; then
    ...
    if [ "$HAS_BINARY" = true ]; then
        ...
        if check_systemd_service; then
            ...
            echo -e "  📦 HermesDeckX: ${GREEN}v${CURRENT_VERSION}${NC} ($BINARY_SERVICE_STATUS) at $INSTALLED_LOCATION"
        else
            echo -e "  📦 HermesDeckX: ${GREEN}v${CURRENT_VERSION}${NC} at $INSTALLED_LOCATION"
        fi
    fi
...
if [ "$HAS_BINARY" = true ]; then
    N=$((N + 1)); MENU_ITEMS+=("$N) Manage local HermesDeckX / 管理本机 HermesDeckX"); MENU_ACTIONS+=("manage_binary")
fi

if [ "$IS_CONTAINER" = false ]; then
    if [ "$HAS_BINARY" = false ]; then
        N=$((N + 1)); MENU_ITEMS+=("$N) Install HermesDeckX locally / 在本机安装 HermesDeckX"); MENU_ACTIONS+=("install_binary")
    fi
...
case "$SELECTED_ACTION" in
    ...
    manage_binary)
        ...
        echo ""
        echo -e "${GREEN}✓ Local HermesDeckX / 本机 HermesDeckX${NC}"
        ...
        case $BCHOICE in
            ...
            2)
                if [ "$SERVICE_RUNNING" = true ]; then
                    ...
                    echo -e "${BLUE}Starting systemd service... / 正在启动 systemd 服务...${NC}"
                    if [ "$SYSTEMD_SERVICE_TYPE" = "user" ]; then
                        systemctl --user start hermesdeckx
                    else
                        sudo systemctl start hermesdeckx
                    fi
                    sleep 2
                    if [ "$SYSTEMD_SERVICE_TYPE" = "user" ]; then
                        if systemctl --user is-active --quiet hermesdeckx 2>/dev/null; then
...
if [ "$(id -u)" = "0" ]; then
    ...
    # 检测 hermesagent 用户是否已存在
    if id "hermesagent" &>/dev/null; then
        echo -e "${GREEN}✓ User 'hermesagent' already exists / 用户 'hermesagent' 已存在${NC}"
        # Ensure hermesagent is in docker group if Docker is installed
        if getent group docker &>/dev/null; then
            if ! id -nG hermesagent 2>/dev/null | grep -qw docker; then
                usermod -aG docker hermesagent
                echo -e "${GREEN}✓ Added 'hermesagent' to docker group / 已将 'hermesagent' 添加到 docker 组${NC}"
            else
                echo -e "${GREEN}✓ 'hermesagent' is already in docker group / 'hermesagent' 已在 docker 组中${NC}"
            fi
        fi
        echo ""
        ...
        echo ""
        echo -e "  ${GREEN}su - hermesagent${NC}"
        echo -e "  ${GREEN}curl -fsSL https://raw.githubusercontent.com/HermesDeckX/HermesDeckX/main/install.sh | bash${NC}"
        echo ""
        exit 0
    fi
    ...
    echo ""
    echo -n "Auto-create user 'hermesagent'? / 自动创建用户 'hermesagent'？ [Y/n] "
    read -n 1 -r REPLY </dev/tty
    echo
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        echo ""
        echo -e "${BLUE}Creating user 'hermesagent'... / 正在创建用户 'hermesagent'...${NC}"
        ...
        if command -v adduser &>/dev/null && adduser --help 2>&1 | grep -q -- '--gecos'; then
            adduser --gecos "" --disabled-password hermesagent
        else
            useradd -m -s /bin/bash hermesagent
        fi
        ...
        PASSWD_SET=false
        for i in 1 2 3; do
            echo -e "${YELLOW}Please set password for 'hermesagent' (enter twice):"
            echo -e "请为 'hermesagent' 设置密码（需输入两次）：${NC}"
            if passwd hermesagent </dev/tty; then
                PASSWD_SET=true
                break
            fi
            echo ""
        ...
        if [ "$PASSWD_SET" = false ]; then
            echo -e "${RED}✗ Failed to set password after 3 attempts. Removing user 'hermesagent'..."
            echo -e "✗ 3 次密码设置均失败，正在删除用户 'hermesagent'...${NC}"
            userdel -r hermesagent 2>/dev/null
        ...
        # 配置 NOPASSWD
        echo 'hermesagent ALL=(ALL) NOPASSWD:ALL' > /etc/sudoers.d/hermesagent
        chmod 440 /etc/sudoers.d/hermesagent
        echo -e "${GREEN}✓ Configured NOPASSWD sudo / 已配置免密 sudo${NC}"
        
        # 启用 lingering，使 systemd --user 实例在无登录会话时也能运行
        loginctl enable-linger hermesagent 2>/dev/null || true
        echo -e "${GREEN}✓ Enabled lingering for systemd user services / 已启用 systemd 用户服务持久化${NC}"
        
        echo ""
        echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
        echo -e "${GREEN}✅ User 'hermesagent' created successfully!${NC}"
        echo -e "${GREEN}✅ 用户 'hermesagent' 创建成功！${NC}"
        echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
        echo ""
        echo -e "${YELLOW}Please run the following commands to switch user and re-run:"
        echo -e "请执行以下命令切换用户并重新运行：${NC}"
        ...
        echo -e "  ${GREEN}curl -fsSL https://raw.githubusercontent.com/HermesDeckX/HermesDeckX/main/install.sh | bash${NC}"
        echo ""
        echo -e "${YELLOW}⚠ Note: Use \"su - hermesagent\" (with dash), not \"su hermesagent\"."
        echo -e "  The dash ensures a full login session required for systemd services."
        echo -e "⚠ 注意：请使用 \"su - hermesagent\"（带短横线），不要用 \"su hermesagent\"。"
...
if [[ "$OS" == *"mingw"* || "$OS" == *"cygwin"* ]]; then
    OS="windows"
    ASSET_PATTERN="hermesdeckx-windows-${ARCH}.exe"
    BINARY_NAME="hermesdeckx.exe"
fi
...
echo -e "${GREEN}═══════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "${CYAN}HermesDeckX location / HermesDeckX 位置：${NC} $INSTALLED_BINARY"
...
# Register in PATH so `hermesdeckx` is available globally
if ! command -v hermesdeckx &>/dev/null || [ "$(command -v hermesdeckx 2>/dev/null)" != "$INSTALLED_BINARY" ]; then
    echo -e "${YELLOW}Would you like to add hermesdeckx to PATH for global access?"
    echo -e "是否将 hermesdeckx 添加到 PATH 以便全局使用？${NC}"
...
echo -e "${BLUE}>> Starting HermesDeckX on port $PORT...${NC}"
echo "----------------------------------------"
if [ "$PORT" -ne "$DEFAULT_PORT" ]; then
